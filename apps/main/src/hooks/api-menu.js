 ...
 */
export const useFetchMenuList = (page, size, orders, searchs) => {
 ...
 */
export const useFetchMenuTree = id => {
  const { data, error, loading } = useFetchData(
    `/api/auth/v5/menus/${id}/tree`
  );
  return { data, error, loading };
};
 ...
 * 创建菜单
 */
export const useCreateMenu = (pid, name, path, icon, hideInMenu) => {
 ...
 * 修改数据字典
 */
export const useModifyMenu = (id, name, path, icon, hideInMenu) => {
 ...
 * @returns
 */
export const useMoveMenu = (menuId, destId, place, relation) => {
