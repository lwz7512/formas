import { usePostData, useDeleteData, vanillaPostData } from '.';
/**
 * 获取文件列表
 */
export const useFecthFilesList = (page, size, orders, searchs) => {
 ...
 * 下载外站链接文件
 */
export const useDownloadWeblink = url => {
...
/**
 * 上传文件块 in an asynchronous way:
 */
export const uploadBolb = async tempcell => {
  return vanillaPostData('/api/sys/files/upload-blob', {
    tempcell: tempcell,
  });
};

const ajaxBlobUpload = async (tempcell, callback) => {
  const res = await uploadBolb(tempcell);
  if (!res) return console.error(`## No response after upload blob!`);
  // here we go!
  switch (res.errCode) {
    case 200:
...
export const cutFile = (filetype, file, callback, bizid) => {
  ...
  for (let i = 0; i < blobTotalNum; i++) {
    ...
    console.log(
      file.name + ' ' + i + '/' + blobTotalNum + ' (' + start + '-' + end + ')'
    );
